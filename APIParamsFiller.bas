...
Public Sub Initialize(engineName As String,preferencesMap As Map)
	...
	frm.RootPane.LoadLayout("apiparamfiller")
	params.Initialize
	originalParams.Initialize
	Dim apimap As Map
	apimap.Initialize
	If preferencesMap.ContainsKey("api") Then
		apimap=preferencesMap.Get("api")
		If apimap.ContainsKey(engineName) Then
			params=apimap.Get(engineName)
...
Sub init(engineName As String)
	...
	Select engineName
		...
		Case "tencent"
			setTableView(Array As String("id","key"), emptyMap)
		Case "youdao"
			setTableView(Array As String("appid","key"), emptyMap)
		Case "mymemory"
